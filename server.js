...
import { mongo_string, port } from './configs.js';
import { sendEmail, hashPassword } from './utils/helper.js';
import { USER } from './models/index.js';

console.log(mongo_string,'mongo_string');
const blueBg = '\x1b[44m\x1b[97m';
const greenBg = '\x1b[42m\x1b[30m';
const redBg = '\x1b[41m\x1b[97m';
const yellowBg = '\x1b[43m\x1b[30m';
const reset = '\x1b[0m';
...
async function ensureAdminExists() {
  try {
    ...
    if (!existingAdmin) {
      ...
      console.log(`${greenBg}✅ Admin user created automatically!${reset}`);
      console.log(`${yellowBg}📧 Email: ${ADMIN_CREDENTIALS.email}${reset}`);
      console.log(`${yellowBg}🔑 Password: ${ADMIN_CREDENTIALS.password}${reset}`);
    } else {
      console.log(`${blueBg}ℹ️  Admin user already exists${reset}`);
    }
  } catch (error) {
    console.error(`${redBg}Error creating admin user:${reset}`, error);
  }
}
...
mongoose
  ...
  .then(async() => {
    console.log(`${greenBg}MongoDB connected${reset}`);
    // Auto-create admin user if not exists
    await ensureAdminExists();
  })
  .catch(err => {
    console.log(`${redBg}Error connecting to MongoDB:${reset}`, err);
  });
...
const allowedOrigins = [
  ...
  'https://launderremedy.com',
  'https://www.launderremedy.com',
  'https://launderremedy-f470d30f2aed.herokuapp.com',
  'https://launderremedy.vercel.app'
];
